# OmniStitch MVP (Chrome Extension)

## 功能
- 在任意博客页面点击扩展图标。
- 自动打开已配置 Agent 站点（ChatGPT / Kimi / DeepSeek / Gemini，可多选）。
- 自动填入“当前 Prompt + 当前页面 URL”。
- 自动点击发送。
- 支持 Prompt 管理（新增/编辑/删除/设为当前）。
- 支持将 AI 回复自动同步到“同步系统”（可切换 provider：关闭 / Webhook / Obsidian）。

## 安装
1. 打开 `chrome://extensions/`。
2. 打开右上角 `Developer mode`。
3. 点击 `Load unpacked`。
4. 选择目录：`omnistitch-mvp`。

## 使用
1. 在扩展详情页点击 `Extension options`，先配置 Prompt（可选）。
   默认内置一个“博客总结” Prompt。
2. 在 `Extension options` 的“同步配置”区域选择 provider：
   - `关闭同步`：只发送，不外部同步。
   - `Webhook`：将结果 POST 到你的 webhook URL。
   - `Obsidian`：通过 Obsidian Local REST API 创建独立 note。
3. 打开任意 `http/https` 博客页面。
4. 点击扩展图标 `OmniStitch MVP`。
5. 等待自动跳转所选 Agent 站点并发送。

## Sync 字段说明
- 扩展内部统一采集字段：
  - `taskid`
  - `agent`（兼容保留 `target`）
  - `time`
  - `aiResponse`
  - `sourceUrl`
- Obsidian provider 为每条 AI 回复创建一个新 note（`PUT /vault/{filename}`），默认写入 `Daily/OmniStitch/YYYY-MM-DD/{article-title}/{agent}/`。

## 回复抓取机制
- 扩展在 Agent 站点标签页注入 main-world 脚本，拦截 `fetch` / `XMLHttpRequest` / `WebSocket` / `EventSource` 的流式文本。
- content script 通过 `window.postMessage` 与 main-world 脚本通信（START/ACK/CHUNK/FINAL）。
- 抓取日志仅输出长度与 100 字符预览，不打印完整正文。

## Prompt 管理
1. 打开 `chrome://extensions/`。
2. 找到 `OmniStitch MVP` 并点击 `Details`。
3. 点击 `Extension options` 进入管理页。
4. 可进行新增、编辑内容、删除、设为当前操作。

## 已知限制
- 需保证你已登录所选 Agent 站点（ChatGPT/Kimi/DeepSeek/Gemini）。
- Agent 站点页面结构变更时，`content.js` 的选择器可能需要更新。
- Agent 站点接口形态变化时，网络流 URL 过滤规则可能需要更新。

## 快捷键一键触发
- 默认快捷键：
  - macOS: `Command+Shift+Y`
  - Windows/Linux: `Ctrl+Shift+Y`
- 修改快捷键：
  1. 打开浏览器扩展快捷键页面（Chrome/Arc 都支持）。
  2. 找到 `OmniStitch MVP` 的 `Send active page URL to configured AI agent`。
  3. 自定义为你习惯的组合键。

## 故障排查
- 点击扩展只跳转不发送：
  1. 在扩展页点击 `Reload` 扩展。
  2. 确认 Agent 站点已登录并且页面可正常手动发送。
  3. 打开扩展的 `service worker` 控制台和 Agent 站点页控制台查看错误日志。
- 同步未生效：
  1. 先确认“同步配置”已保存。
  2. Webhook 模式下检查 URL 可达与服务端返回状态码。
  3. Obsidian 模式下确认 Local REST API 插件已开启，Base URL/API Key 正确。
- 回复未抓取到：
  1. 打开 Agent 站点控制台，搜索 `OMNISTITCH_CAPTURE_ACK`、`response_preview_100`。
  2. 若长期无 ACK，检查扩展是否已授权 `scripting` 权限并重载扩展。
  3. 若有 ACK 但无 FINAL，检查站点是否命中网络过滤规则（日志含 `captureSourceUrl`）。

## E2E 脚本
- 回复链路校验（发送/接收/内容验证）：
  - `node scripts/headed-e2e-response-check.mjs --agent-sites=deepseek`
- 同步存储一致性校验（校验 webhook sync-store 字段完整性 + top/raw/display 一致）：
  - `node scripts/headed-e2e-sync-store-check.mjs --agent-sites=deepseek`
